...
# In this script, we generate the result for multinomial measurement error model for Case 1
...
function error(data1,t1,δ,l,a)
    ...
    for i = 1:length(data1)
        ci = interpr(i-1)
        e = e + data1[i]*log(ci) + (J-data1[i])*log((1-ci))
    end 
    return e
end
...
function sample_para(D1_iv,v1_iv,fmle,M)
    # ------------------------------------------------------------------------------------------------------------------
    # Input:
    # D1_iv, v1_iv: (vector) Intervals for D1 and v1 for rejection sampling.
...
function predicreal(lsp,t)
 # ------------------------------------------------------------------------------------------------------------------
    ...
    for i = 1:length(lsp) 
        ...
        for j = 1:length(xx)
            c1_05 = (quantile(Binomial(J,p(xx[j])),[.05,.95])[1])/J
            c1_95 = (quantile(Binomial(J,p(xx[j])),[.05,.95])[2])/J
            if c1_05 < lb[j] 
                lb[j] = c1_05
            end
...
# Inital guess.
D1g = 0.1; v1g=  0.01
θG = [D1g,v1g] # inital guess
lb=[0.01,0] # lower bound
ub=[0.3,0.1] # upper bound
...
nrange=zeros(1,nptss) # matrix to store the nuisance parameters once optimized out
...
@time for i in 1:nptss
    function fun1(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,t1,δ,L,[D1range[i],aa[1]])
    end
    local lb1=[0] # lower bound 
    local ub1=[0.1] # upper bound
    local θG1=[v1mle] # initial estimate - take the MLE 
...
llstar = -quantile(Chisq(df),0.95)/2 # 95% asymptotic threshold
nptss=40 # points on the profile
v1min=0.02 # lower bound for the profile - can be changed easily
v1max=0.08 # upper bound for the profile - can be changed easily
...
@time for i in 1:nptss
    function fun2(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,t1,δ,L,[aa[1],v1range[i]])
    end
    local lb2=[0.01] # lower bound 
    local ub2=[0.3] # upper bound
    local θG2=[D1mle] # initial estimate - take the MLE 
...
v1_iv = [0.02,0.07]
M = 500
@time lsp =  sample_para(D1_iv,v1_iv,fmle,M)
...
ub = Float64.(ub)
x = 1:δ:200
f = plot(x, lb.*J, lw=0, fillrange=ub.*J, fillalpha=0.40, xlims=(0,200), ylims=(-5,25),
         color=:red, label=false, grid=false, tickfontsize=20, margin=10mm, framestyle=:box,
         xticks=([1,50,100,150,200], [latexstring("1"),"",latexstring("100"),"",latexstring("200")]),
         yticks=([0,5,10,15,20], [latexstring("0"),"","","",latexstring("20")]),linecolor=:transparent)
