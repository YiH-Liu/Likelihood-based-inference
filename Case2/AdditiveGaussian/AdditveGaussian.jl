...
# In this script, we generate the result for additive Gaussian measurement error model for Case 2
...
# error(): Log-likelihood function for the Additive Gaussian Measurement Error Model for Case 2.
function error(data1,data2,δ,L,a)
    #------------------------------------------------------------------------------------------------------------------
    # Input:
    ...
    # a: (vector) Parameter vector.
    # Output:
    # e: (number) Log-likelihood for the Additive Gaussian Measurement Error Model (Case 2) with parameter vector a.
    #------------------------------------------------------------------------------------------------------------------
    ...
    c1 = fc1(0:1:L); c2 = fc2(0:1:L)
    # Estimate the loglikelihood function.
    e=0.0;
    dist1 = Normal(0,a[5]); dist2 = Normal(0,a[6])
    e=loglikelihood(dist1,(data1./J).-c1) + loglikelihood(dist2,(data2./J).-c2)
    return e
end
...
function fun(a)
    ...
    # a: (vector) Parameter vector.
    # Output:
    # error(data1, data2, δ, lx, a): (number) Log-likelihood for the Additive Gaussian Measurement Error Model (Case 2) with parameter vector a.
    # ------------------------------------------------------------------------------------------------------------------
...
function sample_para(D1_iv,D2_iv,v1_iv,v2_iv,σ1_iv,σ2_iv,fmle,M)
    # ------------------------------------------------------------------------------------------------------------------
    # Input:
    # D1_iv, D2_iv, v1_iv, v2_iv, σ1_iv, σ2_iv: (vector) Intervals for D1, D2, v1, v2, σ1, and σ2 for rejection sampling.
    ...
    while count< M
        ...
        ll = fun([D1g,D2g,v1g,v2g,σ1g,σ2g])
        if ll-fmle >= llstar
            ...
            push!(sampledP,(D1g,D2g,v1g,v2g,σ1g,σ2g))
            display(count)
        end
    end
    return sampledP
...
function predicreal(lsp,t)
    ...
    for i = 1:length(lsp)
        ...
        for j = 1:length(xx)
            ulb1 = (quantile(Normal(p1(xx[j]),a[5]),[.05,.95])[1])
            uub1 = (quantile(Normal(p1(xx[j]),a[5]),[.05,.95])[2])
            ulb2 = (quantile(Normal(p2(xx[j]),a[6]),[.05,.95])[1])
            uub2 = (quantile(Normal(p2(xx[j]),a[6]),[.05,.95])[2])
...
v1g=  0.07; v2g=  0.001
σ1g= 0.05; σ2g= 0.11
θG = [D1g,D2g,v1g,v2g,σ1g,σ2g] # inital guess
lb=[0.0001,0.01,-0.1,-0.1,0,0] # lower bound
ub=[0.25,0.25,0.5,0.5,1,1] # upper bound
...
D1min=0.12# lower bound for the profile 
D1max=0.22# upper bound for the profile
...
@time for i in 1:nptss
    function fun1(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,data2,δ,L,[D1range[i],aa[1],aa[2],aa[3],aa[4],aa[5]])
    end
    local lb1=[0.01,-0.1,-0.1,0,0] # lower bound 
    local ub1=[0.25,0.5,0.5,1,1] # upper bound 
    local θG1=[D2mle,v1mle,v2mle,σ1mle,σ2mle] # initial estimate - take the MLE
...
@time for i in 1:nptss
    function fun2(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,data2,δ,L,[aa[1],D2range[i],aa[2],aa[3],aa[4],aa[5]])
    end
    local lb2=[0.0001,-0.1,-0.1,0,0] # lower bound 
    local ub2=[0.25,0.5,0.5,1,1] # upper bound
    local θG2=[D1mle,v1mle,v2mle,σ1mle,σ2mle] # initial estimate - take the MLE 
...
v1max=0.1 # upper bound for the profile 
...
@time for i in 1:nptss
    function fun3(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,data2,δ,L,[aa[1],aa[2],v1range[i],aa[3],aa[4],aa[5]])
    end
    local lb3=[0.0001,0.0001,-0.1,0,0] # lower bound 
    local ub3=[0.25,0.25,0.5,1,1] # upper bound
    local θG3=[D1mle,D2mle,v2mle,σ1mle,σ2mle] # initial estimate - take the MLE 
...
@time for i in 1:nptss
    function fun4(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,data2,δ,L,[aa[1],aa[2],aa[3],v2range[i],aa[4],aa[5]])
    end
    local lb4=[0.0001,0.0001,-0.1,0,0] # lower bound 
    local ub4=[0.25,0.25,0.5,1,1] # upper bound
    local θG4=[D1mle,D2mle,v1mle,σ1mle,σ2mle] # initial estimate - take the MLE 
...
llstar = -quantile(Chisq(df),0.95)/2 # 95% asymptotic threshold
nptss=40 # points on the profile
σ1min=0.025 # lower bound for the profile 
...
@time for i in 1:nptss
    function fun5(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,data2,δ,L,[aa[1],aa[2],aa[3],aa[4],σ1range[i],aa[5]])
    end
    local lb5=[0.0001,0.0001,-0.1,-0.1,0] # lower bound 
    local ub5=[0.25,0.25,0.5,0.5,1] # upper bound
    local θG5=[D1mle,D2mle,v1mle,v2mle,σ2mle] # initial estimate - take the MLE 
...
s3=plot(σ1range,nllσ1,lw=6,xlim=(0,0.2),ylim=(-3,0.1),legend=false,tickfontsize=20,labelfontsize=20,margin = 10mm,grid=false,
xticks=([0,0.1,0.2], [latexstring("0"),latexstring("0.1"),latexstring("0.2")]),yticks=([-3,-2,-1,0],[latexstring("-3"),latexstring("-2"),latexstring("-1"),latexstring("0")]),framestyle=:box,left_margin = 15mm)
...
llstar = -quantile(Chisq(df),0.95)/2 # 95% asymptotic threshold
nptss=40 # points on the profile
σ2min=0.01 # lower bound for the profile  
σ2max=0.15 # upper bound for the profile  
...
@time for i in 1:nptss
    function fun6(aa) # function to return loglikelihood by fixing the interest parameter along the profile
        return error(data1,data2,δ,L,[aa[1],aa[2],aa[3],aa[4],aa[5],σ2range[i]])
    end
    local lb6=[0.0001,0.0001,-0.1,-0.1,0] # lower bound 
    local ub6=[0.25,0.25,0.5,0.5,1] # upper bound
    local θG6=[D1mle,D2mle,v1mle,v2mle,σ1mle]
...
s3=plot!(σ2range,nllσ2,lw=6,xlim=(0,0.2),ylim=(-3,0.1),legend=false,tickfontsize=20,labelfontsize=20,margin = 10mm,grid=false,
xticks=([0,0.1,0.2], [latexstring("0"),latexstring("0.1"),latexstring("0.2")]),yticks=([-3,-2,-1,0],[latexstring("-3"),latexstring("-2"),latexstring("-1"),latexstring("0")]),framestyle=:box,color=1, line=:dot,left_margin = 15mm)
s3=vline!([σ2mle],lw=6,color=3, line=:dot)
s3=hline!([llstar],lw=6,color=2)
s3 = annotate!(0.085, -3.6, text(L"σ_{1}, σ_{2} ", :left, 20))
s3 = annotate!(-0.09/2, -1.5, text(L"\bar{\ell}_{p}", :left, 20))
savefig(s3,"$path\\Figure6e.pdf")  
display(s3) 
...
σ2_iv = [0.085,0.14]
M = 500
@time lsp =  sample_para(D1_iv,D2_iv,v1_iv,v2_iv,σ1_iv,σ2_iv,fmle,M)
...
σ1sampled = [Para[5] for Para in lsp]
σ2sampled = [Para[6] for Para in lsp]
...
savefig(q2,"$path\\inval_P2.pdf")
display(q2)

q3=scatter(v1sampled,legend=false,grid=false,xticks = false,tickfontsize = 15,size=(600, 200),markersize = 3,
...
q4=scatter(v2sampled,legend=false,grid=false,xticks = false,tickfontsize = 15,size=(600, 200),markersize = 3,
ylims = (v2_iv[1]-0.01,v2_iv[2]+0.01),yticks=(v2_iv, [string(v2_iv[1]), string(v2_iv[2])]),markerstrokecolor = :white, color=:black)
q4=hline!(v2_iv,legend=false,lw = 4, linestyle = :dash, color=:blue)
savefig(q4,"$path\\inval_q2.pdf")
display(q4)

q5=scatter(σ1sampled,legend=false,grid=false,xticks = false,tickfontsize = 15,size=(600, 200),markersize = 3,
ylims = (σ1_iv[1]-0.01,σ1_iv[2]+0.01),yticks=(σ1_iv, [string(σ1_iv[1]), string(σ1_iv[2])]),markerstrokecolor = :white, color=:black)
q5=hline!(σ1_iv,legend=false,lw = 4, linestyle = :dash, color=:blue)
savefig(q5,"$path\\inval_sig1.pdf")
display(q5)

q6=scatter(σ2sampled,legend=false,grid=false,xticks = false,tickfontsize = 15,size=(600, 200),markersize = 3,
ylims = (σ2_iv[1]-0.01,σ2_iv[2]+0.01),yticks=(σ2_iv, [string(σ2_iv[1]), string(σ2_iv[2])]),markerstrokecolor = :white, color=:black)
q6=hline!(σ2_iv,legend=false,lw = 4, linestyle = :dash, color=:blue)
savefig(q6,"$path\\inval_sig2.pdf")
display(q6)
...
lb1 = Float64.(lb1); lb2 = Float64.(lb2);
ub1 = Float64.(ub1); ub2 = Float64.(ub2);
x = 1:δ:L+1
...
savefig(f1,"$path\\RpredictC1.pdf")
display(f1)
...
savefig(f2,"$path\\RpredictC2.pdf")
display(f2)
