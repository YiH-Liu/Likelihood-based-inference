...
using FilePathsBase
using LaTeXStrings
# In this script we genrate Figure 2(a)

# Get the path of the current script
path = dirname(@__FILE__)
t=5000

# Read the CSV file: index for subpopulation 1 at initial condition
index_0 = DataFrame(CSV.File("$path\\index_0.csv")); 


# Read the CSV file: Count data for subpopulation 1 at initial condition
data_0 = DataFrame(CSV.File("$path\\data_0.csv")); 
C_0 = data_0.a;

# Read the CSV file: coordinate for subpopulation 1 at time t
index = DataFrame(CSV.File("$path\\index.csv")); 


# Read the CSV file: Count data for subpopulation 1 at time t
...
# Plot the snapshot for subpopulation 1 at t = 5000.
...
f3 = scatter(1:1:200,C_0,xticks=([1,50,100,150,200], [latexstring("1"),"","","",latexstring("200")]),
...
f4 = scatter(1:1:200,C,xticks=([1,50,100,150,200], [latexstring("1"),"","","",latexstring("200")]),
            yticks=([-5,0,5,10,15,20,25],["",latexstring("0"),"","","",latexstring("20"),""]),label=false,xlims = [1,200],ylims = [-5,25],grid = false,
        tickfontsize = 20,color=:red, framestyle=:box,markerstrokecolor=:red,aspect_ratio = :equal)
